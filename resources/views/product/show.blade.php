       ...
       <div class="max-w-2xl mx-auto mt-14 sm:mt-16 lg:max-w-none lg:mt-0 lg:row-end-2 lg:row-span-2 lg:col-span-3">
         <div class="flex flex-col-reverse">
           <div class="mt-4">
             ...
             <h2 id="information-heading" class="sr-only">Product information</h2>
				 <p class="text-md text-gray-700 mt-2">${{ number_format(($product->price / 100), 2, '.', '') }}</p>
             <p class="text-sm text-gray-500 mt-2">{{ $product->created_at->format('M d, Y') }}</p>
           </div>
         ...
         <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
           <button type="button" x-bind:disabled="loadingPurchase"
			  		@click="purchase({{ $product->id }}, '{{ $product->title }}', {{ $product->price }})"
